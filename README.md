...
框架以 SLAM 定位为基础，借助重定位构建完整 TF 树，通过查询 `map → chassis → muzzle` 变换链，实时获取炮口在场地坐标系下的位姿；结合先验目标点，依次完成 **Yaw 轴对准 → Pitch 弹道解算** 的两阶段闭环控制，最终实现在车辆任意点位状态下的自动吊射校准。得益于北极熊战队导航开源框架的泛用性基础，本框架在其之上快速迭代开发完成。
...
<video src="assets/Hero_Shoot.mp4" controls width="100%"></video>
