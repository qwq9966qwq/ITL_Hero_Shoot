...
int main(int argc, char ** argv)
{
  ...
  // [Z_DIAG] 启动时输出所有与Z轴相关的关键参数
  {
    auto log = utils::logger();
    log->info("====== [Z_DIAG_CONFIG] Point_LIO Z轴诊断日志已启用 ======");
    log->info("[Z_DIAG_CONFIG] use_imu_as_input={}, imu_en={}, prop_at_freq_of_imu={}", use_imu_as_input, imu_en, prop_at_freq_of_imu);
    log->info("[Z_DIAG_CONFIG] gravity=({}, {}, {})", gravity[0], gravity[1], gravity[2]);
    log->info("[Z_DIAG_CONFIG] gravity_init=({}, {}, {})", gravity_init[0], gravity_init[1], gravity_init[2]);
    log->info("[Z_DIAG_CONFIG] acc_norm={}, G_m_s2将在后面计算", acc_norm);
    log->info("[Z_DIAG_CONFIG] extrinsic_T=({}, {}, {})", extrinT[0], extrinT[1], extrinT[2]);
    log->info("[Z_DIAG_CONFIG] extrinsic_R=[{},{},{}; {},{},{}; {},{},{}]",
      extrinR[0], extrinR[1], extrinR[2], extrinR[3], extrinR[4], extrinR[5], extrinR[6], extrinR[7], extrinR[8]);
    log->info("[Z_DIAG_CONFIG] extrinsic_est_en={}, check_satu={}", extrinsic_est_en, check_satu);
    log->info("[Z_DIAG_CONFIG] acc_cov_output={}, gyr_cov_output={}, imu_meas_acc_cov={}, imu_meas_omg_cov={}",
      acc_cov_output, gyr_cov_output, imu_meas_acc_cov, imu_meas_omg_cov);
    log->info("[Z_DIAG_CONFIG] lidar_type={}, filter_size_surf={}, filter_size_map={}", lidar_type, filter_size_surf_min, filter_size_map_min);
  }
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (flg_first_scan) {
        ...
        utils::logger()->info("[Z_DIAG_INIT] flg_first_scan: G_m_s2={:.4f}, imu_en={}", G_m_s2, imu_en);
        utils::logger()->info("[Z_DIAG_INIT] kf gravity设置: input=({:.4f},{:.4f},{:.4f}), output=({:.4f},{:.4f},{:.4f})",
          kf_input.x_.gravity(0), kf_input.x_.gravity(1), kf_input.x_.gravity(2),
      ...
      if (!p_imu->after_imu_init_)  // !p_imu->UseLIInit &&
      {
        if (!p_imu->imu_need_init_) {
          ...
          {
            auto log = utils::logger();
            V3D euler_init = SO3ToEuler(rot_init);
            log->info("[Z_DIAG_GRAVITY] 重力对齐完成! tmp_gravity=({:.4f},{:.4f},{:.4f})",
              tmp_gravity(0), tmp_gravity(1), tmp_gravity(2));
            log->info("[Z_DIAG_GRAVITY] 初始欧拉角 roll={:.2f}° pitch={:.2f}° yaw={:.2f}°",
              euler_init(0) * 180.0 / M_PI, euler_init(1) * 180.0 / M_PI, euler_init(2) * 180.0 / M_PI);
            log->info("[Z_DIAG_GRAVITY] kf_output.acc=({:.4f},{:.4f},{:.4f}) (应约等于 -R^T*g)",
              kf_output.x_.acc(0), kf_output.x_.acc(1), kf_output.x_.acc(2));
            log->info("[Z_DIAG_GRAVITY] kf gravity: ({:.4f},{:.4f},{:.4f})",
              kf_output.x_.gravity(0), kf_output.x_.gravity(1), kf_output.x_.gravity(2));
          }
        } else {
          continue;
      ...
      // [DIAG] 每10帧输出一行紧凑状态 — 用于定位漂移发生时刻和原因
      {
        static int z_diag_frame_cnt = 0;
        z_diag_frame_cnt++;
        if (z_diag_frame_cnt % 10 == 0) {
          V3D pos, vel, grav;
          V3D euler;
          if (!use_imu_as_input) {
            pos = kf_output.x_.pos; vel = kf_output.x_.vel; grav = kf_output.x_.gravity;
            euler = SO3ToEuler(kf_output.x_.rot);
          } else {
            pos = kf_input.x_.pos; vel = kf_input.x_.vel; grav = kf_input.x_.gravity;
            euler = SO3ToEuler(kf_input.x_.rot);
          }
          utils::logger()->info("[DIAG] f={} pos=({:.2f},{:.2f},{:.2f}) rpy=({:.1f},{:.1f},{:.1f}) |v|={:.2f} feat={}/{} |g|={:.3f}",
            z_diag_frame_cnt,
            pos(0), pos(1), pos(2),
